...
const ProjectsList: React.FC = () => {
  ...
  // API hooks - use debounced search
  ...
  const {
    ...
    // Skip the query if no filters are set (optional)
    skip: false,
  });
  ...
  useEffect(() => {
    console.log("API Response:", { projectsData, isLoading, error });
  }, [projectsData, isLoading, error]);

  // Debug: Log when filters change and trigger refetch
  useEffect(() => {
    const apiFilters = { ...filters, search: debouncedSearch };
    console.log("Filters changed:", apiFilters);
    console.log("API call should be triggered with:", apiFilters);
    // Manually trigger refetch to ensure API call is made
    refetch();
  ...
  const columns: ColumnsType<Project> = [
    {
      ...
      sorter: true,
      width: "25%",
      render: (name: string, _record: Project) => (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
            ...
            {isAdmin() && (
              <button
                ...
                className="h-10 sm:h-12 px-4 sm:px-6 text-sm sm:text-base rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-medium border-none shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2"
              >
                <PlusOutlined />
                Create Project
              </button>
